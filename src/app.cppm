...
export ProviderPtr create_provider(const std::string& name) {
    if (name == "paru") {
...
export std::vector<std::string> get_available_providers() {
    ...
    auto brew_p = std::make_unique<BrewProvider>();
    if (brew_p->is_available()) {
